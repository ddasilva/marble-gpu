...
class DispersionSimSetup:
    ...
    particle_invlat_axis: np.ndarray
...
def setup_dispersion_sim():
    ...
    particle_invlat_axis = np.deg2rad(np.arange(config.inv_lat_min, config.inv_lat_max, config.inv_lat_step))
    particle_lat_axis = np.arccos(np.sqrt((1 / config.particle_height) * np.cos(particle_invlat_axis)**2))
    ...
    return DispersionSimSetup(
        particle_long_axis,
        particle_invlat_axis,
        particle_lat_axis,
        particle_energy_axis,
        particle_state
    )